...
#### 实战
connect算子可以将两条流数据进行关联, 使用connect只能关联两条数据流, 但是不限制数据类型, 任意数据类型都可以进行关联;
调用connect算子后DataStream会转为ConnectedStreams。

还需要注意的是, 使用connect算子后, 我们要对数据处理的算子map, flatMap等操作不在是由MapFunction或者FlatMapFunction来实现, 而是通过CoMapFunction或者CoFlatMapFunction来实现, 里面会有flatMap1和flatMap2用来处理第一条流数据和第二条流数据, 最先处理哪条数据则更具哪条数据先进入;


我们通过connect来实现两条数据的join操作
