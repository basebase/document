...
我们先来看一下下面这张图。如果我们要进行union(4,2), 4这个节点的根节点是8, 2这个节点的根节点是7。

如果我们是基于size进行优化的话, 4所在的树节点总数是3, 而2所在的树的节点总数是6个。根据上面特性节点小的树指向节点大的树, 合并后也就是8指向7。但是这里需要注意一点就是, 这颗树的深度是"4"。<strong>在合并之前两棵树的深度分别是2和3</strong>,
经过这样的合并之后, 这棵树的深度增加了。

但是在我们这个例子中, 节点数少的这颗树也就是以8为根节点的这棵树, 它的高度反而比根节点为7的树更高。所以, 一个更加合理的合并方案是让7这个节点指向8这个节点。这样合并后得到的新树深度依然是3。对于我们union这个过程来说一个更好的合并方式是在每一个节点上记录一下以这个节点为根的深度。所以, 我们在合并的时候应该使用<strong>"深度比较低的树指向深度比较高的树"</strong>

那么这种优化方案, 被称为基于rank的优化。  
rank[i]表示根节点为i的树的高度。

![1-6](https://github.com/basebase/img_server/blob/master/common/unionfind06.jpg?raw=true)
