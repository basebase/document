...
**我们举的这两个例子, 无论是从空树添加元素还是在已有节点上添加元素, 本质是一样的。"都是插入的元素在不平衡的节点的左侧的左侧", 换句话说, 我们一直在向这棵树的左侧添加元素。最终导致左子树的高度要比右子树的高度要高。与此同时, 我们观察这个不平衡的左子树它的平衡因子也是大于0的, 换句话说对于这个不平衡的节点它的左子树节点高度大于右子树高度的。**
